...
const publicDir = path.resolve((globalThis.process && globalThis.process.cwd ? globalThis.process.cwd() : '.'), 'public')
const svgPath = path.join(publicDir, 'logo.svg')
// write to dist/favicon.ico (Vite default output dir)
const distDir = path.resolve((globalThis.process && globalThis.process.cwd ? globalThis.process.cwd() : '.'), 'dist')
const outPath = path.join(distDir, 'favicon.ico')

async function run(){
  if(!fs.existsSync(svgPath)){
    console.error('logo.svg not found at', svgPath)
    if(globalThis.process && typeof globalThis.process.exit === 'function') globalThis.process.exit(1)
  }

  // ensure dist directory exists; if not, fail with helpful message
  if(!fs.existsSync(distDir)){
    console.error('dist directory not found — run `vite build` before running this script')
    if(globalThis.process && typeof globalThis.process.exit === 'function') globalThis.process.exit(2)
  }
