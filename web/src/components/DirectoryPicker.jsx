...
// - disabled: optional
export default function DirectoryPicker({ value, onChange, label, disabled }) {
	...
	return (
		<div style={{ marginBottom: '0', width: '100%' }}>
			{label && <label style={{ fontWeight: 500, marginRight: 8 }}>{label}</label>}
			<div style={{ position: 'relative', display: 'flex', alignItems: 'center', width: '100%' }}>
				...
				<span
					...
					style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', cursor: disabled ? 'not-allowed' : 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}
					title="Browse Server"
				>
			...
			{showBrowser && (
				<div style={{ position: 'absolute', zIndex: 1000, background: '#222', color: '#eee', border: '1px solid #444', borderRadius: 8, padding: '1rem', minWidth: 320, boxShadow: '0 2px 12px #0008', marginTop: 8 }}>
					...
					{loading ? <div>Loading...</div> : error ? <div style={{ color: '#f44' }}>{error}</div> : (
						<>
