  ...
  return <>
    ...
    {(extra.Status === 'failed' || extra.Status === 'rejected' || extra.Status === 'unknown' || extra.Status === 'error') && (
      <div style={{ position: 'absolute', top: 8, left: 8, zIndex: 2 }}>
        <IconButton
          ...
          onClick={event => handleRemoveBan({ event, extra, baseType, baseTitle, mediaType, media, setExtras, showToast })}
    ...
    {extra.YoutubeId && !downloaded && !imgError && !isFallback && (
      <div style={{ position: 'absolute', top: 8, right: 8, zIndex: 2 }}>
        <IconButton
          icon={<DownloadIcon isDownloading={isDownloading} isQueued={isQueued} />}
          title={getDownloadButtonTitle({ rejected, extra, isDownloading, isQueued })}
...
function DownloadIcon({ isDownloading, isQueued }) {
  ...
  if (isQueued) return <FontAwesomeIcon icon={faClock} color="#fff" size="lg" />;
  ...
  fetch('/api/blacklist/extras/remove', {
    ...
    .then(() => {
      ...
      if (typeof setExtras === 'function' && media !== undefined && media.id) {
        fetch(`/api/${mediaType === 'movie' ? 'movies' : 'series'}/${media.id}/extras`)
