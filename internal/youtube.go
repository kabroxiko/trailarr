...
func AddToDownloadQueue(item DownloadQueueItem, source string) {
	...
	// If source is "task", previously we waited for the queue to be ready which
	// could block long-running sync tasks (making a quick sync take hours). That
	// behavior made scheduled syncs slow because they would wait for the full
	// download processing to complete. Instead, for task-originated enqueues we
	// should not block — just append the item and let the queue worker process it
	// in the background. Keep this non-blocking to ensure sync tasks finish quickly.
...
func createSuccessMetadata(info *downloadInfo, youtubeId string) (*ExtraDownloadMetadata, error) {
	...
	// Add/update the extra in the unified collection in the persistent store
	entry := ExtrasEntry{
		MediaType:  info.MediaType,
		MediaId:    info.MediaId,
		ExtraTitle: info.ExtraTitle,
